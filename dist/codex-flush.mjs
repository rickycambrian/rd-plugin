...
function writeFileAtomic(filePath, body) {
  fs3.mkdirSync(path3.dirname(filePath), { recursive: true });
  const tmp = `${filePath}.tmp.${process.pid}.${Math.random().toString(36).slice(2)}`;
  fs3.writeFileSync(tmp, body, { mode: 384 });
  fs3.renameSync(tmp, filePath);
}
...
function sha2562(input) {
  return createHash5("sha256").update(input).digest();
}
function stableHash(input) {
  return sha2562(input).toString("hex");
}
...
function deterministicId(kind, parts) {
  return uuidV54(`${kind}:${parts.map((p) => String(p)).join(":")}`, KG_NAMESPACE3);
}
function deterministicExecutionId(kind, parts) {
  return uuidV54(`${kind}:${parts.map((p) => String(p)).join(":")}`, EXECUTION_KG_NAMESPACE3);
}
...
function collectFilePaths(input, output = /* @__PURE__ */ new Set()) {
  ...
  for (const [key, value3] of Object.entries(input)) {
    ...
    if (typeof value3 === "string" && /(^|_)(file|path|filepath|filename)$/.test(lowerKey) && value3.length > 0 && value3.length < 1e3) {
      output.add(value3);
    } else {
      collectFilePaths(value3, output);
    }
  }
  return output;
}
...
function eventData(event, contentArtifacts) {
  return {
    ...
    lastAssistantMessageHash: event.lastAssistantMessage ? stableHash(event.lastAssistantMessage) : void 0,
...
function buildCodexHookTraceWriteBundle(trace) {
    ...
    if (event.toolName) {
      ...
      operations.push({
        ...
        properties: {
          ...
          event_index: value(event.sequence),
          tool_input: value(event.toolInput === void 0 ? void 0 : summarizePayload(event.toolInput)),
          tool_response: value(event.toolResponse === void 0 ? void 0 : summarizePayload(event.toolResponse)),
          command: value(extractCommand(event.toolInput) ? summarizeCommand(extractCommand(event.toolInput)) : void 0),
          schema_version: value(TRACE_SCHEMA_VERSION)
        }
      }, {
        operation: "create_edge",
      ...
      addCodeFileOperations(operations, toolNodeId, [...collectFilePaths(event.toolInput), ...collectFilePaths(event.toolResponse)]);
      addCommandOperation(operations, toolNodeId, extractCommand(event.toolInput));
    } else {
      addCodeFileOperations(operations, eventId, [...collectFilePaths(event.toolInput), ...collectFilePaths(event.toolResponse)]);
      addCommandOperation(operations, eventId, extractCommand(event.toolInput));
    }
    const decisionKind = event.decisionKind ?? (event.toolName && /askuser|ask_user/i.test(event.toolName) ? "ask_user" : event.hookEventName.toLowerCase().includes("permission") ? "tool_permission" : null);
    if (decisionKind) {
      ...
      const observation = buildDecisionObservationOperations({
        ...
        selectedOption: event.decisionAnswer ?? (event.toolResponse === void 0 ? void 0 : stableJson(event.toolResponse)),
        actor: "human",
...
function signErc8128Request2(input) {
  ...
  const base = buildSignatureBase({ method: input.method, path: path8, authority, created, expires, nonce, keyid });
...
function recordFailure(full, entry, error, kind, deadDir) {
  ...
  if (attempts >= maxAttempts) {
    try {
      fs5.mkdirSync(deadDir, { recursive: true });
      fs5.renameSync(full, path5.join(deadDir, path5.basename(full)));
      log("warn", "queue entry dead-lettered", { file: path5.basename(full), attempts, error: error.slice(0, 200) });
      return "deadLettered";
    } catch {
    }
  }
  try {
    ...
    fs5.writeFileSync(full, JSON.stringify(updated), { mode: 384 });
  } catch {
  }
  return "retained";
}
function splitEntry(dir, full, entry, operations) {
  try {
    ...
    const tsPrefix = path5.basename(full).split("-")[0] || `${Date.now()}`;
    const rand = Math.random().toString(36).slice(2, 10);
    halves.forEach((ops, i) => {
      ...
      fs5.writeFileSync(path5.join(dir, name), JSON.stringify(request), { mode: 384 });
    });
...
function countQueue(dir) {
  try {
    return fs5.readdirSync(dir).filter((f) => f.endsWith(".json")).length;
  } catch {
    return 0;
  }
}
async function drainQueue(auth, limit = 500, options = {}) {
  ...
  try {
    const stat = fs5.statSync(lockPath2);
  ...
  try {
    fs5.writeFileSync(lockPath2, JSON.stringify({ pid: process.pid, startedAt: (/* @__PURE__ */ new Date()).toISOString() }), { mode: 384 });
  } catch {
  }
  ...
  try {
    ...
    for (const file of files) {
      ...
      let entry;
      try {
        entry = JSON.parse(fs5.readFileSync(full, "utf8"));
      } catch {
        try {
          fs5.rmSync(full, { force: true });
        } catch {
...
function groupTurns(events) {
  ...
  for (const event of events) {
    ...
    if (last && (key === void 0 || last.turnId === key)) {
      if (key !== void 0 && last.turnId === void 0) last.turnId = key;
...
// src/codex/graph.ts
function buildCodexGraphWriteBundle(walletAddress, traces) {
  const operations = [];
  const contentArtifacts = [];
...
function writeCodexSpool(spoolDir, traces) {
  const written = [];
  for (const trace of traces) {
...
// src/codex/flush-core.ts
function codexFingerprint(codexSessionId, sink, events) {
...
async function flushCodexDirect(config, codexSessionId, events) {
  ...
  try {
    ...
    log("warn", "codex derive failed; queueing graph only", { sessionId: codexSessionId, error: err.message });
  }
...
main().catch((err) => {
  try {
    log("error", "codex flush failed", { error: err.message });
  } catch {
  }
