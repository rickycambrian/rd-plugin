...
// src/monitor-advisory.ts
var USAGE = `usage: node monitor-advisory.mjs
...
`;
var ADVISORY_TIMEOUT_MS = 500;
function advisoryTextFromResponse(json) {
  ...
  if (Array.isArray(obj.advisories)) {
    const texts = obj.advisories.map(pick).filter((t) => Boolean(t));
    if (texts.length) return texts.join("\n\n");
  }
  return void 0;
}
async function fetchAdvisory(homeUrl, sessionId, token, timeoutMs = ADVISORY_TIMEOUT_MS) {
...
async function main() {
  ...
  if (config.private_key) {
    try {
      token = await mintHomeWalletToken(config.private_key);
    } catch {
    }
  }
...
main().catch((err) => {
  try {
    log("debug", "monitor-advisory failed (fail-open)", { error: err.message });
  } catch {
  }
}).finally(() => {
  process.exitCode = 0;
