...
// src/lib/http.ts
async function requestJson(url, body, headers2, timeoutMs = 15e3, method = "POST") {
...
function postJson(url, body, headers2, timeoutMs = 15e3, method = "POST") {
...
async function getDeriveHeaders(config) {
  ...
  if (config.apiKey) headers2["X-KF-API-Key"] = config.apiKey;
  const challengeRes = await fetch(`${base}/api/v1/auth/derive-challenge`, { method: "POST", headers: headers2 });
  if (!challengeRes.ok) {
