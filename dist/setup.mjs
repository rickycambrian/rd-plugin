...
function applyWalletVerification(config, verification) {
  if (verification.ok) {
...
function encodeField(fieldType, value, types) {
  switch (fieldType) {
    ...
    default:
      if (types[fieldType]) return hashStruct(fieldType, value, types);
      return encodeIntField(value);
  }
}
...
function deriveKeyLocally(signatureHex) {
  ...
  return crypto2.createHash("sha256").update(Buffer.from(sig, "hex")).digest("hex");
}
...
// src/setup.ts
var USAGE = `usage: node setup.mjs [--status] [key=value ...] [--force] [--skip-verify]
...
`;
async function verifyWallet(config, privateKey) {
  const apiUrl = typeof config.api_url === "string" && config.api_url ? config.api_url : DEFAULT_API_URL;
  const apiKey = typeof config.api_key === "string" ? config.api_key : void 0;
  try {
...
`);
  ...
  const persistedPrivateKey = result.applied.includes("private_key") && typeof finalConfig.private_key === "string" ? finalConfig.private_key : void 0;
